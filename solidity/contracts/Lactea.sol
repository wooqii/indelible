...
contract Lactea is LacteaToken {
    LacteaToken public token;
    bytes32[] public postIds;
    mapping(bytes32 => Post) internal posts;
    Vote[] internal recentVotes;

    event Posting(bytes32 indexed id, address indexed author, string title,  string body, uint256 votes, uint256 timestamp);
    ...
    function create(string title, string body, uint256 timestamp) public {
        ...
        emit Posting(post.id, post.author, post.title, post.body, post.votes, post.timestamp);
    }
